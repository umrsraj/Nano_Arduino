...
String A = "GET /IoTProjects/FootStepPower/footstep.php?a=";
String Z = " HTTP/1.1";
...
void setup()
{
  ...
  lcd.begin(16, 2);
  lcd.clear();
  lcdstring(0,0,"    ADVANCED    ");
  lcdstring(0,1," FOOTSTEP POWER ");
  delay(2000);
...
pinMode(voltage_sensor,INPUT);
  ...
  Serial.println(F("You're connected to the network"));
    ...
    if (client.connect(server1, 80)) {
      Serial.println(F("Connected to server"));
      // Make a HTTP request
      client.println("GET /api.php?username=Mydreamacd&password=Mydre@m123&to=9014449822&from=MYDREM&message=KIT%20STARTED HTTP/1.1");
...
if(voltage < 7)
{
  ...
  lcd.clear ();
  lcdstring(0,0,"LOW VOLTAGE ALERT");
...
Serial.print(F("charging status = "));Serial.println(charging_status);
...
//    client.println(msg+"9014449822"+"&from=MYDREM&message="+"BATTERY%20VOLTAGE="+String(voltage)+"CHARGING%20STATUS="+charging_status+Z);
...
// *****************************************
        ...
        iot = A + state + "&b=" + String(voltage) + Z;
